...
function transformMarks(id, request){
  var converter = new showdown.Converter();
  if(document.getElementById("codeT" + id)){
    sourceTinyMCEhiddenInput = $("input[name="+id+"]"),
    targetArea = document.getElementById("codeT" + id);
    text = sourceTinyMCEhiddenInput.val(),
    data = converter.makeHtml(text);
    targetArea.innerHTML = data;
    if(tinymce.activeEditor.getParam("twShowdownSettings",{}).twMathJaxDisable !== 1){
    ...
    buttons: [
      {
        ...
        onclick: function() {
          ...
          var storedHeight =  $('.codeT.' + targetid).data("height");
          $('.codeT.' + targetid + ', .coderSNparent.' + targetid).css({"max-width":editorWidth, "height":storedHeight})
...
function popCodeShowdown(target) {
  var twPoppedTitle = tinymce.get(target).getParam("twShowdownSettings",{}).twPoppedTitle || "Showdown Markdown HTML Preview";
...
tinymce.PluginManager.add('twShowdown', function(editor) {
  ...
  function initShowdown(justPop) {
    ...
    // else if (!$('.coderSNparent.' + editor.id).length) {
      ...
      var editorWidth = $('#'+editor.id).width();
      $('.codeT.' + editor.id + ', .coderSNparent.' + editor.id).css({"max-width":editorWidth, "height":twInlineHeight}).fadeIn();
      transformMarks(editor.id);
  ...
  function loadAllshowDown(style) {
    ...
    if (typeof showdown == 'undefined') {
      ...
      if (typeof MathJax == 'undefined' && editor.getParam("twShowdownSettings",{}).twMathJaxDisable !== 1){
